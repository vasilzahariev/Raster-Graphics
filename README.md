...
| *void run(std::istream& in = std::cin, std::ostream& out = std::cout)* | Пуска основната функционалност на програмата | ***std::istream& in*** -> входният поток (по подозиране е std::cin)<br />***std::ostream& out*** -> изходният поток (по подозиране е std::cout) |
